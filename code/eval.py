...
# Parameters
data_folder = '/home/yiming_hao/Projects/ImageCaptioning/2019.4.17/image-captioning-bottom-up-top-down/final_dataset'  # folder with data files saved by create_input_files.py
data_name = 'coco_5_cap_per_img_5_min_word_freq'  # base name shared by data files

sampleratio = int(argv[1])
sampleepoch= int(argv[2)
path = './' + str(sampleratio) + '/' #'./30/'
checkpoint_file = path + 'BEST_' + str(sampleepoch) + '_' + str(sampleratio) + '.pth.tar' #'BEST_36_30.pth.tar'  # model checkpoint
...
def evaluate(beam_size):
    ...
    f1 = open(checkpoint_file[:-7]+'.ref.txt', 'w')
    f2 = open(checkpoint_file[:-7]+'.hypo.txt', 'w')
    for i in references:
        for j in i:
            f1.write(j+'\n')
    for i in hypotheses:
